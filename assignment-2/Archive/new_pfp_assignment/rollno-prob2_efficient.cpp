...
struct word_tracker 
{
  alignas(CLS) uint64_t word_count[20 * STRIDE];
...
void print_counters() 
{
  ...
  {
    std::cout << "Thread " << id << " counter: " << tracker.word_count[id * STRIDE] << '\n';
  }
}
...
int main(int argc, char* argv[]) 
{
  ...
  for (int i = 0; i < thread_count; i++) tracker.word_count[i * STRIDE] = 0;
...
void* thread_runner(void* th_args) 
{
  ...
  tracker.word_count[thread_id * STRIDE] += word_count;
