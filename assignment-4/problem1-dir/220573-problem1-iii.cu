...
// Version (iii): Optimized kernel with loop unrolling and permutation
__global__ void opt_kernel(double* in, double* out, int n) {
  ...
  // Use different loop ordering for better memory access pattern
  // Process in z-y-x order for column-major-like access
  ...
  // Load with coalesced memory access
  if (gx < n && gy < n && gz < n) {
    tile[lx + 1][ly + 1][lz + 1] = in[idx_base + gz];
  }
  
  // Unroll halo loading for better instruction-level parallelism
  // X-direction halos
  if (lx == 0) {
    if (gx > 0) tile[0][ly + 1][lz + 1] = in[(gx - 1) * n * n + gy * n + gz];
  } else if (lx == blockDim.x - 1) {
    if (gx < n - 1) tile[TILE_SIZE_X + 1][ly + 1][lz + 1] = in[(gx + 1) * n * n + gy * n + gz];
  }
  
  // Y-direction halos
  if (ly == 0) {
    if (gy > 0) tile[lx + 1][0][lz + 1] = in[gx * n * n + (gy - 1) * n + gz];
  } else if (ly == blockDim.y - 1) {
    if (gy < n - 1) tile[lx + 1][TILE_SIZE_Y + 1][lz + 1] = in[gx * n * n + (gy + 1) * n + gz];
  }
  
  // Z-direction halos
  if (lz == 0) {
    if (gz > 0) tile[lx + 1][ly + 1][0] = in[idx_base + (gz - 1)];
  } else if (lz == blockDim.z - 1) {
    if (gz < n - 1) tile[lx + 1][ly + 1][TILE_SIZE_Z + 1] = in[idx_base + (gz + 1)];
  }
  
  __syncthreads();
  
  // Compute with manually unrolled operations
  if (gx >= 1 && gx < n - 1 && gy >= 1 && gy < n - 1 && gz >= 1 && gz < n - 1) {
    // Use registers to reduce shared memory accesses
    double sum = 0.0;
    sum += tile[lx][ly + 1][lz + 1];       // i-1
    sum += tile[lx + 2][ly + 1][lz + 1];   // i+1
    sum += tile[lx + 1][ly][lz + 1];       // j-1
    sum += tile[lx + 1][ly + 2][lz + 1];   // j+1
    sum += tile[lx + 1][ly + 1][lz];       // k-1
    sum += tile[lx + 1][ly + 1][lz + 2];   // k+1
    
    out[idx_base + gz] = 0.8 * sum;
  }
}
...
int main() {
  ...
  cudaEventRecord(start);
  opt_kernel<<<gridSize, blockSize>>>(d_in, d_out, N);
  cudaEventRecord(end);
  cudaEventSynchronize(end);
